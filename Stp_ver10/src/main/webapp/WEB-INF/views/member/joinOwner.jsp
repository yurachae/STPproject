...
<script>
	...
	function idOverlap() {
		...
		var email = $('#email').val();
		console.log("아이디 var 값 : " + email);
		$.ajax({
			type : 'POST',
			url : 'idOverlapOwner', // 컨트롤러에서 사용 할 주소값
			data : {
				email : email
			},
	...
	function joinCancel() {
		...
		$.ajax({
			...
			success : function(data) { // 통신의 성공을 의미
				if (data == 1) {
					console.log("중복확인 아이디 삭제 완료");
				} else {
					console.log("중복확인 아이디 삭제 실패");
				}
			},
			error : function() { // 통신의 실패를 의미
...
<body class="bg-dark">
	<div class="container">
		<div class="card mx-auto mt-5">
			<div class="card-header" style="text-align:center">기업 회원 가입</div>
			<div class="card-body">
				<form name="joinUserForm" id="joinUserForm" action="joinUserAction" method="post">
					...
					<input type="hidden" name="prkplceno" id="prkplceno" value="0">
